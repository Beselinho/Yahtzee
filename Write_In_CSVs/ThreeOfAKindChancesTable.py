...
filename = "CSV_Tables/TableToaK.csv"
...
def uniqueCombinationDice0(combination, halfFull):
    ...
    if not halfFull:
        for finalCombination in outcomeZero:
            hitToaK = checkThreeOfAKind(finalCombination)
            if hitToaK:
                totalHits += 1
    ...
    for pKD1, pKD2, pKD3, pKD4 in allPossible4DiceKept:
        ...
        if not halfFull:
            for finalDice in outcomeFour:
                ...
                hitToaK = checkThreeOfAKind(partialCombination)
                if hitToaK:
                    totalHits += 1
...
def expectiMaxForTable(combination, halfFull):
    defaultChance = checkThreeOfAKind(combination)
    combination.sort()
    ...
    if numberOfDices == 5:
        for i in range(1, len(combination) - 1):
            if combination[i] == combination[i-1] and combination[i] == combination[i + 1]:
                keptPositions.append(i-1)
